...
func (s *Service) Create(ctx context.Context, in domain.CreateInput) mo.Result[*domain.Product] {
	...
	result := s.repo.Create(ctx, p)
	if !result.IsError() {
		created := result.MustGet()
		s.publisher.Publish(ctx, event.Event{
			Type:       domain.EventProductCreated,
			OccurredAt: time.Now().UTC(),
			Payload:    domain.ProductCreated{ProductID: created.ID.Hex(), SKU: created.SKU, Name: created.Name, Price: created.Price},
		})
	}
	return result
}
...
func (s *Service) Activate(ctx context.Context, id string) mo.Result[*domain.Product] {
	...
	result := s.repo.Activate(ctx, oid)
	if !result.IsError() {
		s.publisher.Publish(ctx, event.Event{
			Type:       domain.EventProductActivated,
			OccurredAt: time.Now().UTC(),
			Payload:    domain.ProductActivated{ProductID: result.MustGet().ID.Hex()},
		})
	}
	return result
}
...
func (s *Service) Deactivate(ctx context.Context, id string) mo.Result[*domain.Product] {
	...
	if !result.IsError() {
		s.publisher.Publish(ctx, event.Event{
			Type:       domain.EventProductDeactivated,
			OccurredAt: now,
			Payload:    domain.ProductDeactivated{ProductID: result.MustGet().ID.Hex(), DeactivatedAt: now},
		})
	}
	return result
}
